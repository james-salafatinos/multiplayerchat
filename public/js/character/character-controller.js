...
export class BasicCharacterController {
  ...
  _Init(params) {
    ...
    const playerTypeForLog =
      params.isLocalPlayer === false ? "REMOTE" : "LOCAL";
    console.log(
      `[NET-ANIM-DBG] ${playerTypeForLog} BasicCharacterController _Init: entity ${params.entityId}, initialIsMoving: ${params.initialIsMoving}, _pendingInitialState set to: ${this._pendingInitialState}`
    );
    ...
    loader.load(
      ...
      (fbx) => {
        ...
        if (!this._params.scene) {
          console.error(
            `[NET-ANIM-DBG] ${playerType} BasicCharacterController._LoadModels: No scene provided for entity ${entityId}!`
          );
          return;
        }

        // IMPORTANT FIX: Add entity ID to userData for context menu detection
        // Propagate entityId to the model AND all its children for proper raycasting
        this._target.userData.entityId = entityId;

        // Traverse all child meshes and add the entityId to their userData as well
        this._target.traverse((child) => {
          child.userData.entityId = entityId;
        });

        console.log(
          `[TRADE-FIX] Setting userData.entityId=${entityId} on character model and all children for ${playerType} player`
        );
        ...
        manager.onProgress = (url, itemsLoaded, itemsTotal) => {
          console.log(
            `[NET-ANIM-DBG] ${playerType} Animation loading progress for entity ${entityId}: ${itemsLoaded}/${itemsTotal} - ${url}`
          );
        };
        ...
        manager.onLoad = () => {
          ...
          console.log(
            `[NET-ANIM-DBG] ${playerType} BasicCharacterController._LoadModels: ALL ANIMATIONS LOADED for entity ${entityId}. Setting FSM state to: ${this._pendingInitialState}`
          );
          console.log(
            `[NET-ANIM-DBG] ${playerType} Animation count: ${
              Object.keys(this._animations).length
            }, Available animations: ${Object.keys(this._animations).join(
              ", "
            )}`
          );
          ...
          if (isRemote) {
            console.log(
              `[NET-ANIM-FIX] LoadingManager.onLoad completed for REMOTE player ${entityId}`
            );
          }
        };
        ...
        manager.onError = (url) => {
          console.error(
            `[NET-ANIM-DBG] ${playerType} Animation loading error for entity ${entityId}: ${url}`
          );
        };
        ...
        const _OnLoad = (animName, anim) => {
          if (anim.animations && anim.animations.length > 0) {
            ...
            console.log(
              `[NET-ANIM-DBG] ${playerType} Animation '${animName}' loaded successfully for entity ${entityId}`
            );
            ...
            if (isRemote) {
              ...
              const hasAllEssential = essentialAnimations.every(
                (name) =>
              ...
              if (hasAllEssential && !this._animationsLoaded) {
                console.log(
                  `[NET-ANIM-FIX] ${playerType} All essential animations loaded for entity ${entityId}. Setting _animationsLoaded=true`
                );
                this._animationsLoaded = true;
            ...
            console.warn(
              `[NET-ANIM-DBG] ${playerType} Animation ${animName} loaded from ${modelPath} has no animation data for entity ${entityId}`
            );
          }
        };
        ...
        console.log(
          `[NET-ANIM-DBG] ${playerType} Loading animations for entity ${entityId}: ${Object.keys(
            animationFiles
          ).join(", ")}`
        );
          ...
          if (animationFiles.hasOwnProperty(animName)) {
            ...
            animLoader.load(
              ...
              (error) => {
                // onError callback
                console.error(
                  `[NET-ANIM-DBG] ${playerType} Error loading animation ${animName} from ${animationFiles[animName]} for entity ${entityId}:`,
                  error
                );
              }
            );
      ...
      (error) => {
        console.error(
          `[NET-ANIM-DBG] ${playerType} Error loading character model for entity ${entityId}:`,
          error
        );
      }
    );
